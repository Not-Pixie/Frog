...
# ajuste DB_HOST/DB_PORT/DB_USER se necessário
DB_HOST=${DB_HOST:-db}
...
# Por segurança, você pode também garantir create_all em dev:
# python -c "from app.database.database import Base, engine; import app.models; Base.metadata.create_all(bind=engine)"

# finalmente, executa o comando padrão do container (por ex. gunicorn/flask run)
exec "$@"
